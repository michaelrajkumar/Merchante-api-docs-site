...
export type SubMerchantCredentials = {
  clientId: string
  clientSecret: string
  username: string
  password: string
...
const STORAGE_KEY = 'merchantE.credentials.subMerchant.v1'

const CERT_SERVER = 'https://merchante-solutions--cert.my.salesforce.com'

export function useSubMerchantCredentials() {
  const [clientId, setClientId] = useState('')
  const [clientSecret, setClientSecret] = useState('')
  const [username, setUsername] = useState('')
  const [password, setPassword] = useState('')
  const [accessToken, setAccessToken] = useState('')
  const [serverUrl, setServerUrl] = useState(CERT_SERVER)
  ...
  // save
  useEffect(() => {
    const obj: SubMerchantCredentials = { clientId, clientSecret, username, password, accessToken, serverUrl }
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(obj))
      window.dispatchEvent(new CustomEvent('merchantE:subMerchantCreds', { detail: obj }))
    } catch {
      // ignore
    }
  ...
  return {
    ...
    clear: () => {
      ...
      setPassword('')
      setAccessToken('')
      setServerUrl(CERT_SERVER)
      try {
        localStorage.removeItem(STORAGE_KEY)
      } catch {}
      try {
        window.dispatchEvent(
          new CustomEvent('merchantE:subMerchantCreds', {
            detail: { clientId: '', clientSecret: '', username: '', password: '', accessToken: '', serverUrl: CERT_SERVER },
          })
        )
      } catch {}
    },
...
// Environment selector for Sub-Merchant
export function SubMerchantEnvironmentSelector({
  serverUrl,
  onChange,
}: {
  serverUrl: string
