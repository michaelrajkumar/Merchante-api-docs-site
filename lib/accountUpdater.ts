...
export const ACCOUNT_UPDATER_OPS: AccountUpdaterOp[] = [
  {
    slug: 'upload',
    title: 'Upload Request File',
    method: 'POST',
    path: '/ausUpload',
    tag: 'File Operations',
    description: 'Upload AUS request file containing account data',
  },
  {
    slug: 'status',
  ...
  },
  {
    ...
    path: '/ausDownload',
    tag: 'File Operations',
    description: 'Download response file using rspfId from status inquiry',
  },
  {
    slug: 'file-formats',
    title: 'File Formats',
...
export const ACCOUNT_UPDATER_TAG_ORDER = ['File Operations', 'Documentation']
...
export function parseAccountUpdaterSpec(yamlText: string) {
  return parseYamlSafe(yamlText)
}

export function groupOpsByTag(ops: AccountUpdaterOp[]): Map<string, AccountUpdaterOp[]> {
  const groups = new Map<string, AccountUpdaterOp[]>()
  for (const tag of ACCOUNT_UPDATER_TAG_ORDER) {
    groups.set(tag, [])
  }
  for (const op of ops) {
