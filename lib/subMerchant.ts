...
export const SUB_MERCHANT_OPS: SubMerchantOp[] = [
  {
    slug: 'auth',
  ...
  },
  {
    ...
    method: 'POST',
    path: '/services/apexrest/merchant-application-payfac/',
    tag: 'Applications',
    description: 'Create a new or update an existing sub-merchant application',
  },
  {
    slug: 'upload-attachments',
    title: 'Upload Attachments',
    method: 'POST',
    path: '/services/data/v54.0/composite/tree/Attachment',
    tag: 'Attachments',
    description: 'Upload supporting documents for an application',
  },
  {
    slug: 'get-status',
    title: 'Get Application Status',
    method: 'GET',
    path: '/services/apexrest/merchant-application-status/{opportunityId}',
    tag: 'Status',
    description: 'Retrieve the current status of a sub-merchant application',
  },
]

export const SUB_MERCHANT_TAG_ORDER = [
  'Authentication',
  'Applications',
  'Attachments',
  'Status',
...
export function parseSubMerchantSpec(yamlText: string) {
  return parseYamlSafe(yamlText)
}

export function groupOpsByTag(ops: SubMerchantOp[]): Map<string, SubMerchantOp[]> {
  const groups = new Map<string, SubMerchantOp[]>()
  for (const tag of SUB_MERCHANT_TAG_ORDER) {
    groups.set(tag, [])
  }
  for (const op of ops) {
