...
export const BATCH_PROCESSING_OPS: BatchProcessingOp[] = [
  {
    slug: 'upload',
    ...
    path: '/bpUpload',
    tag: 'Batch Upload',
    description: 'Upload a batch of transactions to ME for processing',
  },
  {
    slug: 'download',
    ...
    path: '/bpDownload',
    tag: 'Batch Download',
    description: 'Download batch response file to determine transaction statuses',
  },
]

export const BATCH_PROCESSING_TAG_ORDER = ['Batch Upload', 'Batch Download']
...
export function parseBatchProcessingSpec(yamlText: string) {
  return parseYamlSafe(yamlText)
}

export function groupOpsByTag(ops: BatchProcessingOp[]): Map<string, BatchProcessingOp[]> {
  const groups = new Map<string, BatchProcessingOp[]>()
  for (const tag of BATCH_PROCESSING_TAG_ORDER) {
    groups.set(tag, [])
  }
  for (const op of ops) {
