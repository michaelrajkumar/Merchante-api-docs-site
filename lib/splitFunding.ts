...
export const SPLIT_FUNDING_OPS: SplitFundingOp[] = [
  {
    slug: 'funding-amounts',
    title: 'Create Funding Amounts',
    method: 'POST',
    path: '/funding-amounts',
    tag: 'Funding Amounts',
    description: 'Create or modify merchant funding amounts for transactions',
  },
]

export const SPLIT_FUNDING_TAG_ORDER = ['Funding Amounts']
...
function resolveSchema(doc: any, schema: any, seen = new Set<any>()): any {
  ...
  if (Array.isArray(schema.oneOf)) {
    // For oneOf, merge all variants into a union-like object
    const out: any = {
      type: 'object',
      properties: {},
      required: [] as string[],
    ...
    for (const variant of schema.oneOf) {
      const r = resolveSchema(doc, variant, seen)
...
export function parseSplitFundingSpec(yamlText: string) {
  return parseYamlSafe(yamlText)
}

export function groupOpsByTag(ops: SplitFundingOp[]): Map<string, SplitFundingOp[]> {
  const groups = new Map<string, SplitFundingOp[]>()
  for (const tag of SPLIT_FUNDING_TAG_ORDER) {
    groups.set(tag, [])
  }
  for (const op of ops) {
