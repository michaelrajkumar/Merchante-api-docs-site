...
const OP_TO_PATH: Record<string, { path: string; method: string }> = {
  checkout: { path: '/checkout/sign-in', method: 'post' },
  'credit-card-response': { path: '/webhooks/response-credit-card', method: 'post' },
  'ach-response': { path: '/webhooks/response-ach', method: 'post' },
}
...
export async function GET(req: Request, ctx: { params: Promise<{ op: string }> }) {
  ...
  const filename = `hosted-payments.${op}`
