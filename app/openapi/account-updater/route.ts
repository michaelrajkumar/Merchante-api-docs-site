...
function stripSecurity(doc: any) {
  ...
  if (out?.components?.securitySchemes) delete out.components.securitySchemes
...
export async function GET(req: Request) {
  ...
  if (format === 'json') {
    ...
    return new NextResponse(jsonOut, {
      ...
      headers: {
        ...
        ...(download ? { 'content-disposition': 'attachment; filename="account-updater.json"' } : {}),
      },
    })
  }
  ...
  return new NextResponse(yamlOut, {
    ...
    headers: {
      ...
      'cache-control': 'no-store',
      'x-content-type-options': 'nosniff',
      ...(download ? { 'content-disposition': 'attachment; filename="account-updater.yaml"' } : {}),
    },
  })
}
