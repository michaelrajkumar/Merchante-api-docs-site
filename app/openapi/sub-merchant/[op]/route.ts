...
const OP_TO_PATH: Record<string, { path: string; method: string }> = {
  ...
  'create-application': { path: '/services/apexrest/merchant-application-payfac/', method: 'post' },
  'upload-attachments': { path: '/services/data/v54.0/composite/tree/Attachment', method: 'post' },
  'get-status': { path: '/services/apexrest/merchant-application-status/{opportunityId}', method: 'get' },
}
...
export async function GET(req: Request, ctx: { params: Promise<{ op: string }> }) {
  ...
  const filename = `sub-merchant.${op}`
