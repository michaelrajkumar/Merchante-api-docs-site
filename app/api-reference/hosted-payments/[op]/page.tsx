...
import {
  HOSTED_PAYMENTS_OPS,
  HOSTED_PAYMENTS_TAG_ORDER,
  getSchemaTree,
  schemaToExample,
  parseHostedPaymentsSpec,
  getParameterGroups,
  groupSchemaNodesByParameterGroups,
  type SchemaNode,
} from '@/lib/hostedPayments'
import { HostedPaymentsClient } from './HostedPaymentsClient'
...
export default async function HostedPaymentsOpPage({ params }: Props) {
  const { op } = await params
  const current = HOSTED_PAYMENTS_OPS.find((o) => o.slug === op)
  if (!current) {
    return (
  ...
  const fullSpecUrl = `/openapi/hosted-payments?download=1`
  const fullSpecJsonUrl = `/openapi/hosted-payments?download=1&format=json`
  const opSpecUrl = `/openapi/hosted-payments/?download=1`
  const opSpecJsonUrl = `/openapi/hosted-payments/?download=1&format=json`

  const filePath = path.join(process.cwd(), 'public', 'openapi', 'hosted-payments.yaml')
  const yamlText = await readFile(filePath, 'utf8')
  const spec = parseHostedPaymentsSpec(yamlText)
  ...
  const baseUrl = (spec?.servers?.[0]?.url as string) || 'https://merchante-solutions.com/hpp'
  ...
  // Build code samples
  const examplePayload = opObj?.requestBody?.content?.['application/x-www-form-urlencoded']?.examples?.allOptionalFieldsSent?.value || {
  ...
  // Webhooks are server-to-server callbacks - don't show code samples for merchant to invoke
  const isWebhook = current.slug === 'webhooks-response-credit-card' || current.slug === 'webhooks-response-ach'
  const samples = isWebhook ? [] : buildFormPostSamples({
    servers: spec.servers,
  ...
  if (requestTree && requestSchema) {
    ...
    if (requestSchema.$ref) {
      const refMatch = requestSchema.$ref.match(/^#\/components\/schemas\/(.+)$/)
      if (refMatch) {
        const schemaName = refMatch[1]
        const referencedSchema = spec?.components?.schemas?.[schemaName]
        if (referencedSchema) {
          parameterGroups = getParameterGroups(spec, referencedSchema)
        }
      }
    }
  ...
  // Parse all responses (200, 302, 400, etc.)
  const responses: {
    code: string
    description?: string
    schema?: any
    ...
    if (tree && schema) {
      ...
      if (schema.$ref) {
        ...
        if (refMatch) {
          ...
          const referencedSchema = spec?.components?.schemas?.[schemaName]
          if (referencedSchema) {
            respGroups = getParameterGroups(spec, referencedSchema)
          }
        }
      }
      // Check for x-parameterGroups on inline schema
  ...
  return (
    <HostedPaymentsClient
      operations={HOSTED_PAYMENTS_OPS}
      currentOp={current}
      miniSpec={miniSpec}
      codeSamples={samples}
      tagOrder={HOSTED_PAYMENTS_TAG_ORDER}
                ...
                <div key={resp.code} className="rounded-xl border border-omise-border p-4">
                  <div className="flex items-center gap-2">
                    <span
                      className={clsx(
                        'rounded-full px-2.5 py-1 text-xs font-semibold',
                        resp.code.startsWith('2') || resp.code.startsWith('3')
