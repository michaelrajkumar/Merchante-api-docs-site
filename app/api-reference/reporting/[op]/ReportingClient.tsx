...
import { useReportingCredentials, ReportingEnvironmentSelector } from '@/components/ReportingCredentialsBox'
...
function applyServerToCode(code: string, serverUrl: string) {
  return code
    .replaceAll('https://test.merchante-solutions.com', serverUrl)
    .replaceAll('https://www.merchante-solutions.com', serverUrl)
}

export function ReportingClient({ operations, currentOp, miniSpec, codeSamples, tryItOutGroups, exampleValues, children }: Props) {
  const { creds, setServerUrl } = useReportingCredentials()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = `${creds.serverUrl}${currentOp.path}`
    ...
    <>
      ...
      <div className="p-4 border-b border-omise-border">
        <div className="flex items-center justify-between">
          <div className={clsx(sidebarCollapsed && 'lg:hidden')}>
            <div className="text-xs font-semibold uppercase tracking-wide text-omise-gray-500">Reporting</div>
      ...
      <nav className={clsx('flex-1 overflow-y-auto p-4', sidebarCollapsed && 'lg:px-2')}>
        <ul className="space-y-1">
          {operations.map((o) => {
            const href = `/api-reference/reporting/${o.slug}`
        ...
        <div className={clsx('mt-5 border-t border-omise-border pt-4 text-xs text-omise-gray-500', sidebarCollapsed && 'lg:hidden')}>
          Download transaction reports in CSV or XML format for reconciliation.
        </div>
      </nav>
    </>
  )
      ...
      >
        ...
        <div className="p-4 border-b border-omise-border">
          ...
          <div className="space-y-1">
            ...
            <Link href="/api-reference/reporting" className="block rounded-md px-2 py-1.5 text-sm bg-gradient-to-r from-omise-teal to-omise-cyan text-white font-medium">
              Reporting
            </Link>
          </div>
        </div>
