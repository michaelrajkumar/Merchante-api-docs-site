...
import { useSplitFundingCredentials, SplitFundingCredentialsBox, SplitFundingEnvironmentSelector } from '@/components/SplitFundingCredentialsBox'
...
function applyServerToCode(code: string, serverUrl: string) {
  return code
    .replaceAll('https://api.merchante.com', serverUrl)
    .replaceAll('https://sandbox.merchante.com/v1', serverUrl)
}
...
export function SplitFundingClient({
  operations,
  currentOp,
  miniSpec,
  codeSamples,
  ...
  const { creds, setEnvironment } = useSplitFundingCredentials()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = `${creds.serverUrl.replace(/\/$/, '')}${currentOp.path}`
      const method = currentOp.method
      const authString = btoa(`${creds.profileId}:${creds.profileKey}`)
      const headers = {
      ...
      <div className="p-4 border-b border-omise-border">
        <div className="flex items-center justify-between">
          <div className={clsx(sidebarCollapsed && 'lg:hidden')}>
            <div className="text-xs font-semibold uppercase tracking-wide text-omise-gray-500">Split Funding API</div>
        ...
        {Array.from(grouped.entries()).map(([tag, ops]) => (
          <div key={tag} className="mb-6 last:mb-0">
            ...
            <ul className="space-y-1">
              {ops.map((o) => {
                const href = `/api-reference/split-funding/${o.slug}`
      ...
      >
        ...
        <div className="p-4 border-b border-omise-border">
          ...
          <div className="space-y-1">
            ...
            <Link href="/api-reference/split-funding" className="block rounded-md px-2 py-1.5 text-sm bg-gradient-to-r from-omise-teal to-omise-cyan text-white font-medium">
              Split Funding
            </Link>
