...
import { buildJsonPostSamples } from '@/lib/codeSamples'
import { Markdown } from '@/components/Markdown'
import {
  SPLIT_FUNDING_OPS,
  SPLIT_FUNDING_TAG_ORDER,
  getSchemaTree,
  schemaToExample,
  parseSplitFundingSpec,
  getParameterGroups,
  groupSchemaNodesByParameterGroups,
  type SchemaNode,
} from '@/lib/splitFunding'
import { SplitFundingClient } from './SplitFundingClient'
...
export default async function SplitFundingOpPage({ params }: Props) {
  const { op } = await params
  const current = SPLIT_FUNDING_OPS.find((o) => o.slug === op)
  if (!current) {
    return (
  ...
  const fullSpecUrl = `/openapi/split-funding?download=1`
  const fullSpecJsonUrl = `/openapi/split-funding?download=1&format=json`
  const opSpecUrl = `/openapi/split-funding/?download=1`
  const opSpecJsonUrl = `/openapi/split-funding/?download=1&format=json`

  const filePath = path.join(process.cwd(), 'public', 'openapi', 'split-funding.yaml')
  const yamlText = await readFile(filePath, 'utf8')
  const spec = parseSplitFundingSpec(yamlText)
  ...
  const baseUrl = (spec?.servers?.[0]?.url as string) || 'https://api.merchante.com'
  ...
  // Build code samples
  const examplePayload = opObj?.requestBody?.content?.['application/json']?.examples?.twoTransactions?.value || {
    transactions: [
      {
        id: 'ABC12345678901234567890123456789',
        merchantFundingAmounts: [
          { merchantId: '9999999999', amount: '10000' },
          { merchantId: '8888888888', amount: '500' },
        ],
      },
    ],
  }

  const samples = buildJsonPostSamples({
    baseUrl,
    endpointPath: current.path,
    payload: examplePayload,
    headers: { Authorization: 'Basic <base64(profileId:profileKey)>' },
  })
  ...
  // Parse all responses (200, 400, 401, 403, 500, etc.)
  const responses: {
    code: string
    description?: string
    schema?: any
  ...
  return (
    <SplitFundingClient
      operations={SPLIT_FUNDING_OPS}
      currentOp={current}
      miniSpec={miniSpec}
      codeSamples={samples}
      tagOrder={SPLIT_FUNDING_TAG_ORDER}
      ...
      <div className="rounded-2xl border border-omise-border bg-omise-dark-secondary p-6 shadow-soft-dark">
        ...
        {requestTree && requestTree.nodes.length > 0 && (
          <>
            ...
            <p className="mt-1 text-sm text-omise-gray-400">Sample payload with starter values. Replace with your own data.</p>
            {examplePayload && (
                ...
                <div key={resp.code} className="rounded-xl border border-omise-border p-4">
                  ...
                  {resp.example && (
                    <>
                      ...
                      <pre className="mt-2 rounded-xl border border-omise-border bg-omise-dark p-4 text-xs text-omise-gray-100 overflow-x-auto">
